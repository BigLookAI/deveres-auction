...
class SorPreSaleAdvice(models.Model):
        ...
        return {
            ...
            'context': {
                ...
                'default_template_id': self.env.ref(
                    'sor_auction_documents.mail_template_sor_pre_sale_advice',
                ).id,
            },
        }
    ...
    def action_bulk_send(self):
        ...
        if not to_send:
            raise UserError(_('No unsent Pre-Sale Advices selected.'))
        ...
        sent, skipped = bulk_send_via_template(
            to_send, template,
            _('Pre-Sale Advice not sent: %s has no email on file.'),
        )
        sent.write({'state': 'sent'})
        return bulk_send_notification(
            len(sent), len(skipped), _('Pre-Sale Advice(s)'), not_eligible_count,
        )
